...
form.addEventListener('submit', async (event) => {
  ...
  const { data: createdData, error } = await supabase.functions.invoke('admin-create-user', { body: values })
  if (error) {
    let detail = ''
  ...
  else { message.textContent = 'Acesso criado. Entregue a senha ao funcionário com segurança.'; form.reset(); await refreshTeamList({ id: createdData?.profileId, company_id: globalThis.__sessionProfile?.company_id, full_name: values.name, role: values.role, active: true }); setTimeout(() => modal.classList.add('hidden'), 1800) }
...
async function refreshTeamList(extraProfile = null) {
  const list = document.querySelector('.permission-list')
  if (!list || !can(currentRole(), 'manageUsers')) return
  list.setAttribute('aria-busy', 'true')
  list.innerHTML = '<p class="muted">Carregando equipe…</p>'
  const companyId = globalThis.__sessionProfile?.company_id
  if (!companyId) { list.innerHTML = '<p class="muted">Equipe indisponível no momento.</p>'; list.setAttribute('aria-busy', 'false'); return }
  const { data, error } = await supabase.from('profiles').select('id, full_name, role, active').eq('company_id', companyId).eq('active', true).order('created_at')
  if (error) { list.innerHTML = '<p class="muted">Não foi possível carregar a equipe.</p>'; list.setAttribute('aria-busy', 'false'); return }
  const profiles = [...(data ?? [])]
