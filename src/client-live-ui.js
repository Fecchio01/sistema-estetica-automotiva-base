...
import { chooseSuggestedResponsible } from './operational-automation.js'
...
async function refreshServiceOptions() {
  ...
  const peopleSelect = form.querySelector('[name="responsibleId"]'); if (peopleSelect && profile()?.company_id) { const { data } = await supabase.from('profiles').select('id, full_name, role').eq('company_id', profile().company_id).eq('active', true).order('full_name'); const people = data || []; const selectedId = peopleSelect.dataset.manuallySelected === 'true' ? peopleSelect.value : ''; const responsibleId = chooseSuggestedResponsible(selectedId, globalThis.__operationalAutomation?.suggestedResponsibleId, people); peopleSelect.innerHTML = `<option value="">Selecione um responsável</option>${people.map((person) => `<option value="${escapeHtml(person.id)}" ${person.id === responsibleId ? 'selected' : ''}>${escapeHtml(person.full_name)} · ${escapeHtml(roleLabel(person.role))}</option>`).join('')}`; peopleSelect.dataset.suggested = responsibleId && responsibleId === globalThis.__operationalAutomation?.suggestedResponsibleId ? 'true' : 'false' }
...
globalThis.__prepareServiceSubmission = () => { const form = document.querySelector('#service-form'); if (!form) return; form.dataset.requestId = crypto.randomUUID(); form.dataset.submitting = 'false'; const responsible = form.querySelector('[name="responsibleId"]'); if (responsible) delete responsible.dataset.manuallySelected; globalThis.__lastServiceSubmission = null }
