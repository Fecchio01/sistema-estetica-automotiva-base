...
import { buildWorkloadSuggestion, chooseSuggestedResponsible } from './operational-automation.js'

import { chooseVehicle } from './order-assistant.js'
import { clearOrderDraft, refreshOrderAssistant, restoreOrderDraft } from './order-assistant-ui.js'
...
async function refreshServiceOptions() {
  ...
  if (vehicleSelect && clientSelect) { const selected = records.find((record) => record.id === clientSelect.value); const previousVehicle = vehicleSelect.value; vehicleSelect.disabled = !selected?.vehicles.length; vehicleSelect.innerHTML = `<option value="">${selected?.vehicles.length ? 'Selecione o veículo' : 'Nenhum veículo cadastrado'}</option>${(selected?.vehicles || []).map((vehicle) => `<option value="${escapeHtml(vehicle.id)}">${escapeHtml([vehicle.make, vehicle.model, vehicle.license_plate].filter(Boolean).join(' · '))}</option>`).join('')}`; vehicleSelect.value = chooseVehicle(selected?.vehicles || [], previousVehicle) }
  const peopleSelect = form.querySelector('[name="responsibleId"]'); if (peopleSelect && profile()?.company_id) { const people = globalThis.__teamProfiles || []; const suggestion = buildWorkloadSuggestion(globalThis.__liveServices || [], people).suggestedResponsibleId; const selectedId = peopleSelect.dataset.manuallySelected === 'true' ? peopleSelect.value : ''; const responsibleId = chooseSuggestedResponsible(selectedId, suggestion, people); peopleSelect.innerHTML = `<option value="">Selecione um responsável</option>${people.map((person) => `<option value="${escapeHtml(person.id)}" ${person.id === responsibleId ? 'selected' : ''}>${escapeHtml(person.full_name)} · ${escapeHtml(roleLabel(person.role))}</option>`).join('')}`; peopleSelect.dataset.suggested = responsibleId && responsibleId === suggestion ? 'true' : 'false' }
...
document.addEventListener('change', (event) => { if (event.target.matches('#service-form [name="responsibleId"]')) event.target.dataset.manuallySelected = 'true'; if (event.target.matches('#service-form [name="clientId"]')) { const record = (globalThis.__clientRecords || []).find((item) => item.id === event.target.value); const select = document.querySelector('#service-form [name="vehicleId"]'); if (!select) return; select.disabled = !record?.vehicles.length; select.innerHTML = `<option value="">${record?.vehicles.length ? 'Selecione o veículo' : 'Nenhum veículo cadastrado'}</option>${(record?.vehicles || []).map((vehicle) => `<option value="${escapeHtml(vehicle.id)}">${escapeHtml([vehicle.make, vehicle.model, vehicle.license_plate].filter(Boolean).join(' · '))}</option>`).join('')}`; select.value = chooseVehicle(record?.vehicles || []) } })

globalThis.__prepareServiceSubmission = () => { const form = document.querySelector('#service-form'); if (!form) return; form.dataset.requestId = crypto.randomUUID(); form.dataset.submitting = 'false'; const responsible = form.querySelector('[name="responsibleId"]'); if (responsible) delete responsible.dataset.manuallySelected; globalThis.__lastServiceSubmission = null; refreshServiceOptions(); restoreOrderDraft(form) }
...
document.addEventListener('submit', async (event) => {
  ...
  try { const totalAmount = totalForCatalogServices(globalThis.__serviceCatalog || [], services); const createdOrder = await createWorkOrder(profile(), { id: requestId, clientId: values.clientId, vehicleId: values.vehicleId, responsibleId: values.responsibleId, services, totalAmount, status: 'scheduled', scheduledAt: null }); globalThis.__lastServiceSubmission = { fingerprint, createdAt: Date.now() }; globalThis.__addLiveWorkOrder?.(createdOrder); document.querySelector('#service-modal').classList.add('hidden'); clearOrderDraft(); form.reset(); button.disabled = false; document.dispatchEvent(new CustomEvent('live-data-refresh-requested')); announce('Atendimento recebido e sincronizando com a operação.') } catch (error) { message.textContent = error.message; button.disabled = false } finally { form.dataset.submitting = 'false'; globalThis.__serviceSubmissionInFlight = false }
}, true)
