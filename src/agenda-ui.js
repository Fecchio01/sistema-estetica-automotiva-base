...
import { buildWorkloadSuggestion } from './operational-automation.js'
...
function renderForm(root) {
  ...
  const suggestedId = buildWorkloadSuggestion(globalThis.__liveServices || [], data.people).suggestedResponsibleId
  const catalog = globalThis.__serviceCatalog || []
  const peopleOptions = data.people.map((person) => `<option value="${escapeHtml(person.id)}" ${person.id === suggestedId ? 'selected' : ''}>${escapeHtml(person.full_name)} · ${escapeHtml(person.role === 'administrator' ? 'Administrador(a)' : person.role === 'reception' ? 'Recepção' : 'Funcionário')}</option>`).join('')
  root.innerHTML = `<div class="agenda-form-panel"><div class="module-toolbar"><div><p class="eyebrow">NOVA RESERVA</p><h2>Reservar horário</h2><small>Escolha os dados reais da empresa para criar a ordem.</small></div><button class="outline-button" id="agenda-form-close">Cancelar</button></div><form id="agenda-booking-form"><label>Cliente<select name="clientId" required><option value="">Selecione um cliente</option>${clientOptions}</select></label><label>Veículo<select name="vehicleId" required disabled><option value="">Selecione o cliente primeiro</option></select></label><label>Serviço<select name="service" required><option value="">Selecione um serviço</option>${catalog.map((service) => `<option value="${escapeHtml(service.name)}">${escapeHtml(service.name)} · ${Number(service.price).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</option>`).join('')}</select></label><label>Responsável<select name="responsibleId" required><option value="">Selecione um responsável</option>${peopleOptions}</select></label><div class="agenda-form-row"><label>Data<input type="date" name="date" min="${today}" required /></label><label>Horário<input type="time" name="time" required /></label></div><p class="auth-message" id="agenda-form-message" role="alert"></p><div class="form-actions"><button type="button" class="outline-button" id="agenda-form-cancel">Cancelar</button><button class="primary-button" type="submit">Salvar reserva</button></div></form></div>`
  const form = root.querySelector('#agenda-booking-form')
  const vehicleSelect = form.querySelector('[name="vehicleId"]')
  form.querySelector('[name="clientId"]').addEventListener('change', (event) => { const vehicles = data.vehicles.filter((vehicle) => vehicle.client_id === event.target.value); vehicleSelect.disabled = !vehicles.length; vehicleSelect.innerHTML = `<option value="">${vehicles.length ? 'Selecione o veículo' : 'Nenhum veículo cadastrado'}</option>${vehicles.map((vehicle) => `<option value="${escapeHtml(vehicle.id)}">${escapeHtml(vehicleLabel(vehicle))}</option>`).join('')}`; vehicleSelect.value = chooseVehicle(vehicles) })
