...
function render(root) {
  ...
  root.querySelectorAll('[data-quote-action="status"]').forEach((select) => select.addEventListener('change', () => { quotes[Number(select.dataset.quoteIndex)] = { ...quotes[Number(select.dataset.quoteIndex)], status: select.value }; render(root) }))
