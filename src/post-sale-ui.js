...
async function render(root) {
  ...
  try {
    ...
    const cards = grouped.map((group, index) => {
      ...
      const details = group.items.map((item) => { const itemGroup = classifyFollowUp(item, now); return `<div class="post-sale-step"><div><span class="post-sale-step-dot ${itemGroup}"></span><div><b>${escapeHtml(typeLabel[item.follow_up_type] || item.follow_up_type)}</b><small>${new Date(item.due_at).toLocaleDateString('pt-BR')} · ${escapeHtml(item.message)}</small></div></div>${item.status === 'pending' ? `<button class="outline-button" data-post-sale-send="${item.id}">Marcar enviado</button>` : '<span class="status-pill ready">Enviado</span>'}</div>` }).join('')
      return `<article class="post-sale-client-card"><button class="post-sale-client-summary" type="button" aria-expanded="${index === 0}" data-post-sale-toggle="${index}"><span class="post-sale-client-avatar">${escapeHtml(client.split(/\s+/).map((part) => part[0]).join('').slice(0, 2).toUpperCase())}</span><span class="post-sale-client-copy"><b>${escapeHtml(client)}</b><small>${escapeHtml(vehicle)}</small></span><span class="post-sale-client-progress"><strong>${group.pendingCount ? `${group.pendingCount} pendentes` : 'Tudo em dia'}</strong><small>${group.items.length} acompanhamentos · ${statusLabel[latestGroup]}</small></span><span class="post-sale-chevron">⌄</span></button><div class="post-sale-client-details ${index === 0 ? '' : 'hidden'}" data-post-sale-details="${index}">${details}</div></article>`
