...
export function buildDashboardAttentionMarkup(model) {
  ...
  const radar = [['Sem responsável', unassigned.length, 'unassigned'], ['Parados', stale.length, 'stale'], ['Retiradas', pickups.length, 'pickups'], ['Fotos', photoPending.length, 'photos'], ['Pós-venda', postSalePending.length, 'post-sale']]
  const radarTotal = radar.reduce((total, [, count]) => total + count, 0)
  const radarSegments = radar.map(([label, count, tone]) => `<span class="dashboard-attention-radar-segment ${tone}" style="width:${radarTotal ? Math.round((count / radarTotal) * 100) : 0}%" aria-label="${label}: ${count}"></span>`).join('')
  const radarLegend = radar.map(([label, count, tone]) => `<span><i class="${tone}"></i>${label}<b>${count}</b></span>`).join('')
  return `<section class="dashboard-panel dashboard-attention-panel"><div class="dashboard-panel-heading"><div><p class="eyebrow">ACOMPANHAMENTO</p><h2>Atenção operacional</h2></div><span class="dashboard-count">${radarTotal}</span></div><p class="muted">Pontos que merecem uma olhada rápida durante o dia.</p><div class="dashboard-attention-radar" role="img" aria-label="Distribuição das pendências operacionais"><div class="dashboard-attention-radar-track">${radarSegments}</div><div class="dashboard-attention-radar-legend">${radarLegend}</div></div><div class="dashboard-attention-grid"><div class="dashboard-attention-block"><div class="dashboard-attention-heading"><b>Sem responsável</b><span>${unassigned.length}</span></div><div class="dashboard-attention-list">${list(unassigned, 'Todas as ordens têm responsável.')}</div></div><div class="dashboard-attention-block"><div class="dashboard-attention-heading"><b>Veículos parados há mais tempo</b><span>${stale.length}</span></div><div class="dashboard-attention-list">${list(stale, 'Nenhum veículo acima de 3h.')}</div></div><div class="dashboard-attention-block"><div class="dashboard-attention-heading"><b>Próximas retiradas</b><span>${pickups.length}</span></div><div class="dashboard-attention-list">${list(pickups, 'Nenhuma retirada pendente.')}</div></div><div class="dashboard-attention-block"><div class="dashboard-attention-heading"><b>Pendência de fotos</b><span>${photoPending.length}</span></div><div class="dashboard-attention-list">${photoList}</div></div><div class="dashboard-attention-block"><div class="dashboard-attention-heading"><b>Mensagens de pós-venda</b><span>${postSalePending.length}</span></div><div class="dashboard-attention-list">${postSaleList}</div></div></div></section>`
}
