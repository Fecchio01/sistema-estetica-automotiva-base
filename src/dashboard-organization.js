...
export function buildDashboardPaddockMarkup(model) {
  ...
  return `<section class="dashboard-panel dashboard-paddock-panel"><div class="dashboard-panel-heading"><div><p class="eyebrow">PÁTIO AO VIVO</p><h2>Onde estão os veículos</h2></div><span class="dashboard-count">${model.rows.length}</span></div><p class="muted">Acompanhe cada veículo, etapa, responsável e tempo desde a entrada.</p><div class="dashboard-stage-cards">${stageCards}</div><div class="dashboard-paddock-list">${rows}</div></section>`
}
