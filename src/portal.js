...
if (isClientPortalPath(window.location.pathname)) {
  ...
  fetch(endpoint).then(async (response) => { const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Link inválido.')
    ...
    const photos = (data.photos || []).filter((photo) => /^https?:\/\//i.test(photo.url || ''))
    const groups = groupPhotosByChecklistStage(photos)
    const photoMarkup = PHOTO_CHECKLIST_STAGES.map(({ id, label, hint }) => `<section class="photo-checklist-stage"><div class="photo-checklist-heading"><div><b>${label}</b><small>${hint}</small></div><span>${groups[id].length} ${groups[id].length === 1 ? 'foto' : 'fotos'}</span></div><div class="photo-grid">${groups[id].map((photo) => `<figure><img src="${escapeHtml(photo.url)}" alt="Foto do veículo na etapa ${label.toLowerCase()}" /><figcaption>${escapeHtml(photo.caption || 'Registro do atendimento')}</figcaption></figure>`).join('') || '<p class="photo-stage-empty">Nenhuma foto registrada nesta etapa.</p>'}</div></section>`).join('') + (groups.general.length ? `<section class="photo-checklist-stage"><div class="photo-checklist-heading"><div><b>Registro geral</b><small>Fotos antigas sem etapa definida.</small></div><span>${groups.general.length} fotos</span></div><div class="photo-grid">${groups.general.map((photo) => `<figure><img src="${escapeHtml(photo.url)}" alt="Foto adicional do veículo" /><figcaption>${escapeHtml(photo.caption || 'Registro do atendimento')}</figcaption></figure>`).join('')}</div></section>` : '')
    content.innerHTML = `<p class="eyebrow">ACOMPANHAMENTO DO VEÍCULO</p><h1>${escapeHtml(data.client.name)}</h1><p class="public-portal-vehicle">${escapeHtml(vehicle)}</p><span class="status-pill in-progress">${statusLabel}</span><section class="public-portal-service"><small>Serviço</small><b>${escapeHtml(data.order.service)}</b>${data.order.scheduledAt ? `<small>Horário: ${new Date(data.order.scheduledAt).toLocaleString('pt-BR', { dateStyle: 'short', timeStyle: 'short' })}</small>` : ''}${data.order.expectedCompletionAt ? `<small>Previsão de entrega: ${new Date(data.order.expectedCompletionAt).toLocaleString('pt-BR')}</small>` : ''}</section><div class="timeline">${timeline}</div><section class="portal-photos public-portal-photos"><div class="portal-section-heading"><div><p class="eyebrow">CHECKLIST VISUAL</p><h3>Fotos por etapa</h3></div><span class="photo-count">${photos.length} ${photos.length === 1 ? 'foto' : 'fotos'}</span></div><div class="photo-checklist">${photoMarkup}</div></section><p class="public-portal-note">Este acompanhamento é atualizado pela equipe da estética.</p>`
  }).catch((error) => { content.innerHTML = `<div class="empty-panel"><h2>Link indisponível</h2><p>${escapeHtml(error.message)}</p><a href="/">Voltar</a></div>` })
}
