...
function renderModule(section, navigationToken = currentNavigationToken) {
  ...
  if (section === 'agenda') {
    ...
    content.innerHTML = `<div class="module-grid"><div class="module-panel"><div class="module-toolbar"><h2>Resumo operacional</h2><button class="outline-button" id="export-report">Exportar CSV</button></div><div class="data-line"><div><b>${money(reportBilling.received)}</b><small>Faturamento recebido no m&ecirc;s</small></div><span class="status-pill ready">Financeiro</span></div><div class="data-line"><div><b>${reportBilling.orderCount}</b><small>Ordens feitas no m&ecirc;s</small></div><span>Tempo real</span></div><div class="data-line"><div><b>${money(reportBilling.outstanding)}</b><small>Valores em aberto</small></div><span class="status-pill waiting">A acompanhar</span></div><div class="data-line"><div><b>${money(reportBilling.averageTicket)}</b><small>Ticket m&eacute;dio do m&ecirc;s</small></div><span>Atualizado</span></div></div><div class="module-panel"><h2>Leitura da opera&ccedil;&atilde;o</h2><div class="data-line"><div><b>${reportCounts.active}</b><small>Ordens em opera&ccedil;&atilde;o</small></div><span class="status-pill in-progress">Abertas</span></div><div class="data-line"><div><b>${reportCounts.ready}</b><small>Prontos para retirada</small></div><span class="status-pill ready">Avisar</span></div><p class="muted">Os indicadores acompanham as ordens e os pagamentos registrados no per&iacute;odo atual.</p><div class="mini-notice"><span class="status-dot green"></span><div><b>Dados atualizados</b><small>O resumo muda quando uma etapa, pagamento ou entrega &eacute; registrada.</small></div></div></div></div>`;
...
function renderDashboardOrganization() {
  ...
  const paddockMarkup = dashboardOrganization ? dashboardOrganization.buildDashboardPaddockMarkup(dashboardOrganization.buildDashboardOrganizationModel(services, serviceStates, globalThis.__teamProfiles || [])) : '';
  const stageChartMarkup = dashboardOrganization ? dashboardOrganization.buildDashboardStageChartMarkup(dashboardOrganization.buildDashboardOrganizationModel(services, serviceStates, globalThis.__teamProfiles || [])) : '';
