...
The migration `20260825_allow_repeat_work_orders.sql` must also be applied to
the test project. Afterward, inserting a second order for the same
`company_id`, `client_id`, and `vehicle_id` must be allowed; pairing a vehicle
with a different client must remain rejected by the composite foreign key.
