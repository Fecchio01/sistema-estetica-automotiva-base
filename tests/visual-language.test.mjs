...
test('linguagem visual compartilhada evita controles quadrados e campos nativos quebrados', () => {
  ...
  assert.match(styles, /\.dashboard-timeline-item,.dashboard-attention-item,.dashboard-stage-card,\.dashboard-summary-metric\{transition:transform \.2s ease,box-shadow \.2s ease,background \.2s ease/)
  assert.match(styles, /\.dashboard-timeline-item:hover,.dashboard-attention-item:hover,.dashboard-stage-card:hover,\.dashboard-summary-metric:hover\{transform:translateY\(-2px\);box-shadow:/)
