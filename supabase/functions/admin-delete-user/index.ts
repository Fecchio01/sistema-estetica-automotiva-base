...
Deno.serve(async (req) => {
  ...
  if (error) return json({ error: `Não foi possível apagar o funcionário: ${error.message}` }, 500)
  const { data: remainingProfile } = await admin.from('profiles').select('id').eq('id', target.id).maybeSingle()
  if (remainingProfile) return json({ error: 'O acesso foi removido, mas o perfil ainda existe. Execute novamente para concluir a limpeza.' }, 500)
  return json({ deleted: true, profileId: target.id, authDeleted: true, profileDeleted: true })
})
