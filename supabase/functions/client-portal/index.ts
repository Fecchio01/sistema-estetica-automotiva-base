...
Deno.serve(async (req) => {
  ...
  const { data: order } = await admin.from('work_orders').select('status, scheduled_at, service_description, vehicle_id, current_stage, expected_completion_at').eq('id', tokenRow.work_order_id).eq('company_id', tokenRow.company_id).eq('client_id', tokenRow.client_id).single()
  ...
  const { data: vehicle } = await admin.from('vehicles').select('make, model, license_plate').eq('client_id', tokenRow.client_id).eq('company_id', tokenRow.company_id).eq('id', order?.vehicle_id || '00000000-0000-0000-0000-000000000000').maybeSingle()
